...
function addEntry(event, didITurnIntoSquirrel) {
  JOURNAL.push({
    events: event,
...
function gatherCorellation(journal) {
  var phis = {};
...
var corellations = gatherCorellation(JOURNAL);

console.log(corellations.pizza);


for (var event in corellations) {
  var corellation = corellations[event];
  if (corellation > 0.1 || corellation < -0.1)
    console.log(event + ": " + corellation);
}
